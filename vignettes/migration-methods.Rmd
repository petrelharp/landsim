...
First, suppose we want migrants to exit at the external boundaries,
...
```{r gaus_mig}
gauss.mig <- migration_matrix(habitat,sigma=100,radius=500,kern="gaussian")
image(gauss.mig)
```

Or, a two-dimensional, symmetric Cauchy kernel:
```{r cauchy_mig}
cauchy.mig <- migration_matrix(habitat,sigma=100,radius=800,kern="cauchy")
image(cauchy.mig)
```
