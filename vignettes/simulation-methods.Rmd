...
```{r pollen}
P <- migrate(N,kernel=pollen.kernel,sigma=pollen.sigma,radius=max.pollen.dist)
this.plot(P)
```
...
```{r disperse_seeds}
SD <- migrate(S,kernel=seed.kernel,sigma=seed.sigma,radius=max.seed.dist,
                     normalize=TRUE)
this.plot(SD)
```
...
```{r get_competition}
K <- migrate(sum(N),kernel=competition.kernel,sigma=competition.sigma,
...
germination_fun <- function (N) {
    germination.args$r0 / ( 1 + migrate(N,germination.args$competition)/germination.args$carrying.capacity )
}
...
```{r init_new_pop, fig.height=2*fig.dim/3}
N <- rpois_raster( brick( habitat, habitat, habitat )/3 )
names(N) <- c("aa","aA","AA")
this.plot(N)
for (k in 1:5) {
    NN <- generation(N,this.demography)
    this.plot(NN)
}
```
