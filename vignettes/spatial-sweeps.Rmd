...
```{r stability_check}
base.r <- intrinsic_growth( pop, this.demography, 
...
```{r high_density}
base.r <- intrinsic_growth( pop, this.demography, density=500,
                         carrying.capacity=values(pop$habitat)[pop$habitable] )
pl(pop$habitat, v=base.r-1)
```
...
Here are snapshots of the simulation:
```{r plot_sim_snapshots}
for (k in seq_along(plot.times)) {
    pl(pop$habitat,v=sim$N[,,k],main=c("",sprintf("t=%d",plot.times[k])))
