...
###### test non-simulation method

### first test at a not-terribly-small value of sigma

sigma <- 0.1
res <- 1
f <- function (fact) { discretize_kernel('gaussian', res=c(res,res), radius=3*sigma, sigma=sigma, fact=fact, sim=FALSE) }
...
####### now test Monte Carlo method

### first test at a not-terribly-small value of sigma

sigma <- 0.1
res <- 1
f <- function (fact) { discretize_kernel('gaussian', res=c(res,res), radius=3*sigma, sigma=sigma, fact=fact, sim=TRUE) }
fact.vec <- c(5,10,15,20,25,30)
names(fact.vec) <- fact.vec
f.list <- lapply( fact.vec, f )
